...
# dos genes ortologos de hsap e rnov
...
for hsapID,rnovID in OrthoDict.items():
    pair1 = hsapID+" "+rnovID
...
def RBH_Filter(RBHFileName,sourceDB):
    ...
    for linha in RBHFile:
        ...
        if "spliceprot" in sourceDB: 
            ...
            if "|" in coluna[1]:
                rnT1,rnEnsT = coluna[1].split("|")
            if "|" not in coluna[1]:
                rnT1 = coluna[1].replace("Rnov_","")
            Tpair = hsT1+" "+rnT1
        ...
        if "refseq" in sourceDB:
            ...
            infoRn = coluna[1].replace("|","-").replace("Rnov_","").split("-")
        ...
        if "ccds" in sourceDB:
            ...
            rnInfo = coluna[1].replace("|","-").replace("Rnov_","").split("-")
...
RBHHeader = "HsGeneID"+"\t"+"RnGeneID"+"\t"+RBH_Header
RBHdict3 ={}
...
for hsT,Info in RBHdict3.items():
    ...
    if type(finalIndex) != str:
        ...
        if hsT in sortDict.keys():
            sortDict[hsT]["rn1"].append(rnTarget[finalIndex])
        ...
        if hsT not in sortDict.keys():
            sortDict[hsT] = {"rn1":[rnTarget[finalIndex]],
...
for human,rbhInfo in sortDict.items():
    ...
    if len(qCoverList2) == 1:
        if qCoverList2[0] >= 60 and tCoverList2[0] >= 90 or\
                ...
                rnID = rnList2[0]
                mouseLista.append(rnID)
                sort2Dict[human] = {"target":rnID,
     ...
     #   if len(indexLista) > 1:
      #      for tcindex in indexLista:
       #         qCoverSubconjA.append(qCoverLista[tcindex])
        ...
        sort2Dict[hsTa] = {"target":rnTargeta[finalIndexA],
...
for human2,info2 in sort2Dict.items():
    ...
    if rn3 in sort3Dict.keys():
        qCover2 = qCover3
        qCover1 = sort3Dict[rn3]["qCover"]
        if qCover2 > qCover1:
            sort3Dict[rn3]["hsT"] = human2 
            sort3Dict[rn3]["qCover"] = qCover2 
            sort3Dict[rn3]["tCover"] = tCover3 
            sort3Dict[rn3]["evalue"] = eValue3 
            sort3Dict[rn3]["bitscore"] = bitScore3 
            sort3Dict[rn3]["genePair"] = GenePair3 
            sort3Dict[rn3]["qualifier"] = qualifier3 
            sort3Dict[rn3]["tEnd"] = tEnd3 
            sort3Dict[rn3]["qEnd"] = qEnd3 
            sort3Dict[rn3]["qStart"] = qStart3 
            sort3Dict[rn3]["tStart"] = tStart3
        if qCover2 == qCover1:
            tCover2 = tCover3
            tCover1 = sort3Dict[rn3]["tCover"]
            if tCover2 > tCover1:
                sort3Dict[rn3]["hsT"] = human2 
                sort3Dict[rn3]["qCover"] = qCover2 
                sort3Dict[rn3]["tCover"] = tCover2 
                sort3Dict[rn3]["evalue"] = eValue3 
                sort3Dict[rn3]["bitscore"] = bitScore3 
                sort3Dict[rn3]["genePair"] = GenePair3 
                sort3Dict[rn3]["qualifier"] = qualifier3 
...
for a2,b2 in sort3Dict.items():
    hsG,rnG = b2["genePair"].split(" ")
    mouseID = a2
    humanID = b2["hsT"]
    info3 = [hsG,rnG,humanID,mouseID,b2["evalue"], b2["bitscore"],b2["qStart"],b2["qEnd"],\
