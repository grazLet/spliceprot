...
pickleOrthoFile = "/home/leticia.costa/disk1/leticia.costa/ortologos_flavia/data/ortho_ensembl_101/Table_ID_Hsap_Rnov_Ortho_1-1.pickle"
...
for database in database_list:
    ...
    # 2.1 app1 Total
    App1TotalFile = open(database+"/diamond_needle/RBH/hsap2/rnov2.rbh").read().split("\n")[:-1]
    App1TotalOutFile = open(App1OutFilePath+database+"_app1_RBH_total.txt","w")
    for line3 in App1TotalFile:
        line3 = line3.replace("#","").replace("Hsap_","").replace("Rnov_","")
    ...
    for line4 in App1File:
        ...
        if App1File.index(line4) != 0:
            ...
            rngene2 = col4[1].strip()
            hsTr2 = col4[2].strip()
            rnTr2 = col4[3].strip()
            appr2 = "rbh"
            rbhInfo = "\t".join(col4[4:-1])
            pair_1 = hsTr2+"\t"+rnTr2
            pair_2 = hsgene2 +"\t"+ rngene2
    ...
    headerFinal = "\t".join(["HsGeneID","RnGeneID","\t".join(needleHeaderItens),"\t".join(rbhHeaderItens[2:]),"method"])
    ...
    App2TotalFile = open(database+"/app2_diamond/RBH/hsap2/rnov2.rbh").read().split("\n")[:-1]
    ...
    for app1Pair in app1_list_3:
        hs,rn = app1Pair.split(",")
        hsApp1List.append(hs)
        rnApp1List.append(rn)
    
    print(">>>>>", database)
    for rbhPair in app2_dif_app1_list_3:
        RBHOnlyFile.write(rbhPair)
        RBHOnlyFile.write("\n")
        hs1,rn1 = rbhPair.split(",")
        if hs1 not in hsApp1List and rn1 not in rnApp1List:
            print(rbhPair)
