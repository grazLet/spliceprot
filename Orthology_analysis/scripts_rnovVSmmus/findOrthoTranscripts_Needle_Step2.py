...
def multiFasta_printDict_function(multiFasta,dbName):
    ...
    return(mmFastaLista,rnFastaLista)




# inicialmente selecionar apenas seqs de mmus e rnov
...
alignheader = "mmTranscriptID"+"\t"+"rnTranscriptID"+"\t"+"similarity"+"\t"+"similarityRatio"+"\t"+"identity"+"\t"+"identityRatio"+"\t"+"gap"+"\t"+"gapRatio"+"\t"+"score"+"\n"
...
for files2 in fileList:
    ...
    # sera associado a mais de um Mmus Transcript ID 
        RnovAlignD = {}
    ...
    # value hs2["fasta"] e hs2["geneID"]
        ...
        for mmFASTA,rnFASTA in itertools.product(mmL,rnL):
        ...
        # criar nome do arquivo q vai guardar resultado do alinhament
                if len(mm1) >= 50:
                    ...
                    mmFilename = folder2+"temp_fasta/"+mm2_new+"_temp.fa"
                        #outFname = folder2+"needle_files/"+mm2_new+"_"+rn1+"_needle.txt"
                if len(mm1) < 50:
                    mm2_new = mm1
                    mmFilename = folder2+"temp_fasta/"+mm2_new+"_temp.fa"
                        #outFname = folder2+"needle_files/"+mm2_new+"_"+rn1+"_needle.txt"
                ...
                if len(rn1) < 50:
                    rn2_new = rn1
                    rnFilename = folder2+"temp_fasta/"+rn2_new+"_temp.fa"
                outFname = folder2+"needle_files/"+mm2_new+"_"+rn2_new+"_needle.txt"


        # rodar o alinhamento needle
                needle_cline = NeedleCommandline(asequence=rnFilename, bsequence=mmFilename, gapopen=10, gapextend=0.5, outfile=outFname)
        ...
        # preencher dict hsAlignDict com os resultados do needle
                ...
                if mm1_original in mmAlignDict.keys():
                        rn1List = []
                        rn1List = mmAlignDict[mm1_original]["rn1"]
                        rn1List.append(rn1_original)
                        mmAlignDict[mm1_original]["rn1"] = rn1List

                        identRatioList = []
                        identRatioList = mmAlignDict[mm1_original]["ident_ratio"]
                        identRatioList.append(ident_ratio)
                        mmAlignDict[mm1_original]["ident_ratio"] = identRatioList

                        simRatioList = []
                        simRatioList = mmAlignDict[mm1_original]["sim_ratio"]
                        simRatioList.append(sim_ratio)
                        mmAlignDict[mm1_original]["sim_ratio"] = simRatioList

                        scoreList = []
                        scoreList = mmAlignDict[mm1_original]["score"]
                        scoreList.append(score)
                        mmAlignDict[mm1_original]["score"] = scoreList

                        gapRatioList = []
                        gapRatioList = mmAlignDict[mm1_original]["gap_ratio"]
                        gapRatioList.append(gap_ratio)
                        mmAlignDict[mm1_original]["gap_ratio"] = gapRatioList
            
                        alignSeqList = []
                        alignSeqList = mmAlignDict[mm1_original]["align_seq"]
                        alignSeqList.append(alignment_seq)
                        mmAlignDict[mm1_original]["align_seq"] = alignSeqList

                        similarityList = []
                        similarityList = mmAlignDict[mm1_original]["similarity"]
                        similarityList.append(similarity)
                        mmAlignDict[mm1_original]["similarity"] = similarityList

                        identityList = []
                        identityList = mmAlignDict[mm1_original]["identity"]
                        identityList.append(identity)
                        mmAlignDict[mm1_original]["identity"] = identityList

                        gapsList = []
                        gapsList = mmAlignDict[mm1_original]["gap"]
                        gapsList.append(gaps)
                        mmAlignDict[mm1_original]["gap"] = gapsList

                if mm1_original not in mmAlignDict.keys():
                        mmAlignDict[mm1_original] = {"rn1":[rn1_original],
        ...
        for mmT1,AlignInfo in mmAlignDict.items():
        ...
        # repetir procedimento para score e ident
                for aa in range(len(AlignInfo["rn1"])):
                        ...
                        alignIDinfoLista2 = [mmT1,AlignInfo["rn1"][aa],AlignInfo["similarity"][aa], \
                        ...
                        fileNeedleOut.write("\n")
                #count += 1
                #msg = mmap sequence number " + str(count)
                #print(msg)
                ...
                if len(indexlist) > 1:
                    ...
                    # como ortologa de mais de uma proteoforma de Mmus                

                if type(finalIndex) != str:
                        if AlignInfo["rn1"][finalIndex] in RnovAlignD.keys():
                                rn1List2 = []
                                rn1List2 = RnovAlignD[AlignInfo["rn1"][finalIndex]]["mmT1"]
                                rn1List2.append(mmT1)
                                RnovAlignD[AlignInfo["rn1"][finalIndex]]["mmT1"] = rn1List2
                        ...
                        if AlignInfo["rn1"][finalIndex] not in RnovAlignD.keys():
                                RnovAlignD[AlignInfo["rn1"][finalIndex]] = {"mmT1": [mmT1],
        ...
        for rnT1,RnovAlignInfo in RnovAlignD.items():
                ...
                if type(finalIndexRn) != str:
                        mmTransId = RnovAlignInfo["mmT1"][finalIndexRn]

                        if mmTransId in ProtOrt.keys():
                                rnT1Lista = []
                                rnT1Lista = ProtOrt[mmTransId]
                                rnT1Lista.append(rnT1)
                                ProtOrt[mmTransId] = rnT1Lista

                        if mmTransId not in ProtOrt.keys():
        
                                ProtOrt[mmTransId] = [rnT1]
                                alignIDinfoLista2 = [RnovAlignInfo["mmT1"][finalIndexRn],rnT1,RnovAlignInfo["similarity"][finalIndexRn], \
...
pickleFileName = folder2+"ortho-pairs_app1/MmRn-ProteoOrtho_v1.pickle"
pickleFileout =  open(pickleFileName, "wb")
pickle.dump(ProtOrt,pickleFileout)
pickleFileout.close()

ProtOrtD = open(folder2+"ortho-pairs_app1/MmRn-ProteoOrtho_v1.txt","w")
