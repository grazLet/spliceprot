...
pickleOrthoFile = "/home/leticia.costa/disk1/ortologos_flavia/data/ortho_ensembl_101/Table_ID_Rnov_Mmus_Ortho_1-1.pickle"
...
for database in database_list:
    ...
    # 2.1 app1 Total
    App1TotalFile = open(database+"/diamond_needle/RBH/rnov2/mmus2.rbh").read().split("\n")[:-1]
    App1TotalOutFile = open(App1OutFilePath+database+"_app1_RBH_total.txt","w")
    for line3 in App1TotalFile:
        line3 = line3.replace("#","").replace("Rnov_","").replace("Mmus_","")
    ...
    headerFinal = "\t".join(["RnGeneID","MmGeneID","\t".join(needleHeaderItens),"\t".join(rbhHeaderItens[2:]),"method"])
    ...
    App2TotalFile = open(database+"/app2_diamond/RBH/rnov2/mmus2.rbh").read().split("\n")[:-1]
    ...
    for rbhPair in app2_dif_app1_list_3:
        ...
        RBHOnlyFile.write("\n")
        rn1,mm1 = rbhPair.split(",")
        if rn1 not in rnApp1List and mm1 not in mmApp1List:
            print(rbhPair)
